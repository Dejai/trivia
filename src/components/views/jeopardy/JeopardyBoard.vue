	...
	<main>
		...
		<div id="jeopardyBoard">
			<div class="columns width-80" style="width: 80%; position: relative" v-if="!isFinalJeopardy">
				<JeopardyColumn
					:style="{ width: mainCategoryWidths }"
					...
					<template #body>
						<div>
							<IconButton @click="onStartGame" class="color-green" style="font-size: 24px">
								...
								<template #content>Start Game</template>
							</IconButton>
						</div>
					</template>
				</ModalView>
			...
			<div class="columns width-80" style="position: relative" v-if="isFinalJeopardy">
				<JeopardyColumn
					:style="{ width: '100%' }"
					v-for="category in finalJeopardy"
					:key="category"
					:category="category"
					@revealed="onColumnRevealed" />
			</div>
			<div id="teamSection" style="width: 18%">
				...
				<div class="teamRows" style="margin-bottom: 5%; width: 60%">
					<div
						class="teamSectionRow flex-row flex-wrap flex-justify-space-between flex-align-center flex-gap-30 color-white"
						v-for="(team, idx) in teamsSorted">
...
<script setup lang="ts">
	...
	import JeopardyColumn from "@views/jeopardy/JeopardyColumn.vue";
...
<style>
	#playFinalJeopardyButtonSection {
		text-align: center;
		padding: 1% 0%;
	}
