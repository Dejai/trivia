<template>
	<main style="display: relative">
		<div
			...
			class="pointer questionCell finalJeopardyCategoryCell"
			:class="{ cellViewed: props.pair._prevOpen }"
			@click="openCell($event)">
				...
				<template #body>
					<div>
						<IconButton @click="onConfirmWagers" class="color-green" style="font-size: 24px">
							...
							<template #content>Yes, Show Question</template>
						</IconButton>
						<IconButton @click="onCancelConfirmWagers($event)" style="font-size: 24px; color: red">
							<template #icon>
								<x-mark-icon />
							</template>
							<template #content>Not Yet</template>
						</IconButton>
					</div>
				</template>
			</ModalView>
...
<style scoped>
	...
	.finalJeopardyCategory.isCategoryVisible {
		visibility: visible;
	}
	.questionSection {
		width: 50%;
