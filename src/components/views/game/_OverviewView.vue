<template>
	<main>
			...
			<FormContainer>
				<FormRow :stacked="true" :class="'gameDetailsInput'">
					...
					<input
						...
						@change="onShowSaveAfterChange"
						@keydown="onShowSaveAfterChange"
						@keyup="updateMenu" />
				</FormRow>
				<FormRow :stacked="true" :class="'gameDetailsInput'">
					<h3>Description</h3>
		...
		<div id="gameSessionsSubSection">
			...
			<p v-if="showSessionErrorMessage" class="color-orange">
				Cannot add/play sessions for this game because it has errors. Check for any errors on the "Questions/Answers" tab
			</p>
			...
			<div id="sessionSection" v-if="showAddNewSession">
				<SessionForm :session="newSession" :isNew="true" @save="onSessionSave" @cancel="hideNewSessionForm" />
			</div>
		</div>
	</main>
</template>
...
<script setup lang="ts">
	...
	import SessionRow from "@views/game/SessionRow.vue";
	import SessionRowHeader from "@views/game/SessionRowHeader.vue";
	import SessionForm from "@views/game/SessionForm.vue";
	import IconButton from "@views/IconButton.vue";
	import FormContainer from "@views/forms/FormContainer.vue";
	import FormRow from "@views/forms/FormRow.vue";
	...
	const visibleSessions = computed(() =>
		isGameAdmin.value ? currentGame.value.Sessions : currentGame.value.Sessions.filter((x: Session) => !x.IsExpired && x.Code != "TEST")
	);
	const sortedSessions = computed(() =>
		visibleSessions.value?.sort((a: any, b: any) => {
	...
	const showNoAvailableSessionMessage = computed(
		() => (isGameAdmin.value && !hasErrors && !hasVisibleSessions.value) || (!isGameAdmin.value && !hasVisibleSessions.value)
	);
