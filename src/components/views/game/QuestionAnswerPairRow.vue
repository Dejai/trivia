<template>
	<tr class="questionRow rowSection pointer" :class="{ isFirstRow: isFirstRow, isLastRow: isLastRow, justModified: justModified }">
		<td class="questionValue clickToEdit">
			...
			<input
				...
				name="dailyDouble"
				v-if="showEditQna"
				@change="onToggleDailyDouble($event)"
				:checked="props.questionAnswerPair.isDailyDouble()" />
		</td>
		<td class="questionText clickToEdit">
			<div class="qnaContentManagement">
				<image-icon
					v-if="questionHasImage && !showEditQna"
					class="color-gold"
					title="This question has an image"
					@click="onPreviewQuestion" />
			...
			<div>
				<input
					class="qnaFormField italic"
					type="text"
					placeholder="Subtext (shown below text in italics)"
					v-model="props.questionAnswerPair.Question.SubText"
					v-if="showQuestionSubText"
					:disabled="!showEditQna" />
			</div>
		...
		<td class="answerText clickToEdit">
			...
			<div>
				<input
					...
					type="text"
					placeholder="Subtext (shown below text in italics)"
					v-model="props.questionAnswerPair.Answer.SubText"
					v-if="showAnswerSubText"
					:disabled="!showEditQna" />
			</div>
			<div class="mediaSection" v-if="showEditQna">
				<div>
	...
	<tr v-if="showPreview">
		<JeopardyCell v-if="!isFinalJeopardy" :pair="qnaPair" :category-name="props.category?.Name" :is-preview="true" @next="onHidePreview" />
		<FinalJeopardyCell v-if="isFinalJeopardy" :pair="qnaPair" :category-name="props.category?.Name" :is-preview="true" @next="onHidePreview" />
	</tr>
</template>
...
<script setup lang="ts">
	...
	import JeopardyCell from "@views/jeopardy/JeopardyCell.vue";
	import FinalJeopardyCell from "@views/jeopardy/FinalJeopardyCell.vue";
	import FloppyDiskIcon from "@icons/FontAwesome/FloppyDiskIcon.vue";
	import PencilIcon from "@icons/FontAwesome/PencilIcon.vue";
	import XMarkIcon from "@icons/FontAwesome/XMarkIcon.vue";
	import TrashCanIcon from "@icons/FontAwesome/TrashCanIcon.vue";
	import ImageIcon from "@icons/FontAwesome/ImageIcon.vue";
	import VolumeIcon from "@icons/FontAwesome/VolumeIcon.vue";
	import UpRightFromSquare from "@icons/FontAwesome/UpRightFromSquareIcon.vue";
	import ArrowDownIcon from "@icons/FontAwesome/ArrowDownIcon.vue";
	import ArrowUpIcon from "@icons/FontAwesome/ArrowUpIcon.vue";
...
<style scoped>
	...
	.qnaOrderIcons .orderIcon {
		margin-right: 7%;
		color: white;
	}
	.isFirstRow .qnaOrderIcons .upIcon {
		color: gray;
	}
	.isLastRow .qnaOrderIcons .downIcon {
		color: gray;
	}
