	...
	<main style="margin-bottom: 3%">
		...
		<table class="categoryQuestionsTable" style="border-spacing: 0px; padding: 1% 0% 0% 2%">
			...
			<tbody class="categorySectionBody">
				<QuestionAnswerPairRow
					v-for="(questionPair, idx) in questionAnswerPairs"
...
<script setup lang="ts">
	...
	import QuestionAnswerPairRow from "@views/game/QuestionAnswerPairRow.vue";
	...
	const questionAnswerPairs = computed(() =>
		isCustomOrder.value
	...
	const showEditAll = computed(
		() => questionAnswerPairs.value.length > 1 && questionsBeingEdited.value < props.category.QuestionAnswerPairs.length
	);
...
<style scoped>
	...
	.categoryTitle .isFirstRow span.upIcon {
		color: gray;
	}
	.categoryTitle span.orderIcon {
		margin-right: 1%;
		color: white;
	}
	...
	.categoryEditSubsection {
		width: 0px;
		opacity: 0;
		overflow: hidden;
		height: 0px;
	}
	.categoryEditSubsection.editingCategory {
		transition:
			width,
			opacity ease 1s;
		width: 55%;
