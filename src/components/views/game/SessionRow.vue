<template>
	<main class="sessionRowContainer">
		...
		<div v-if="showSessionForm">
			<SessionForm :key="props.session?.Code" :session="props.session" @save="onSave" @cancel="onCancel" />
		</div>
	</main>
</template>
...
<script setup lang="ts">
	...
	import ShareFromSquareIcon from "@icons/FontAwesome/ShareFromSquareIcon.vue";
	...
	const expirationDate = computed(() =>
		props.header
			? "Expiration"
			: `${props.session?.Expires?.Month ?? ""} ${props.session?.Expires?.Day ?? ""}, ${props.session?.Expires?.Year ?? ""}`
	);
